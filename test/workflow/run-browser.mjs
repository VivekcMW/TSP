// Never load application/Vite/Playwright config, dotenv, or inherited credentials.
...
if (process.env.WORKFLOW_BROWSER_TESTS !== 'true' || target.host !== '127.0.0.1' || target.port !== 60053 || target.database !== 'thesocialpundit_acceptance_test') {
  throw new Error('Browser acceptance requires WORKFLOW_BROWSER_TESTS=true and the retained isolated 127.0.0.1:60053 target');
}
...
if (process.argv[2] !== '--isolated-child') {
  ...
  Object.assign(env, {
    WORKFLOW_DB_TESTS: 'true', WORKFLOW_BROWSER_TESTS: 'true',
  ...
  const reportPath = resolve(root, 'test-results/workflow-browser-acceptance.json');
  rmSync(reportPath, { force: true });
  const files = ['test/workflow/browser.test.ts'];
  ...
  if (!report.success || report.testResults.length !== 1 || report.numPassedTests !== 2 || report.numFailedTests || report.numPendingTests) process.exitCode = 1;
}