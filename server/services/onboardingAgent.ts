...
import { getAIErrorResponse } from "./openRouter";
...
export type AgentEvent =
  ...
  | { type: "error"; step: AgentStep | null; code: string }
  | { type: "done" };
...
export async function runOnboardingAgent(input: OnboardingAgentRequest, scope: { tenantId: string }, signal: AbortSignal, emit: (event: AgentEvent) => void) {
  ...
  for (const step of steps) {
    try {
      const result = await suggestOnboardingItems({ ...base, step, publications, topics }, scope, signal, message => emit({ type: "progress", step, message }));
      ...
      emit({ type: "error", step, code: getAIErrorResponse(error).body.code });
    }
  }
  emit({ type: "done" });
}
